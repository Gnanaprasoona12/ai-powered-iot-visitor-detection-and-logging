...
print("Webcam Test System (OpenCV LBPH) running... Press ESC to exit")

def send_lcd(msg):
    print(f"[LCD SIMULATOR] {msg}")
...
def send_request(endpoint):
    print(f"[ESP32 SIMULATOR] Activating endpoint: {endpoint}\n")
...
if not cap.isOpened():
    print("ERROR: Cannot open webcam. Make sure your laptop webcam is not in use.")
    exit(1)
...
while True:
    ...
    for (x, y, w, h) in faces:
        ...
        if len(face_samples) > 0:
            ...
            # Distance less than 95 is generally a more lenient match
            if confidence < 95:
    ...
    if len(faces) > 0 and (current_time - last_detection_time > COOLDOWN):
        ...
        if frame_has_resident:
            print(f"\n✅ {best_resident_name} → Access Granted")
            send_lcd("Access Granted")
            send_request("open")
            log_event(best_resident_name, "GRANTED", "-", image_path)
        else:
            ...
            if user_input == str(otp):
                print("✅ OTP Correct - Access Granted")
                send_lcd("Access Granted")
                send_request("open")
    ...
    cv2.imshow("Smart Visitor System - Local Setup", frame)
